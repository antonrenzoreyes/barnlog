...
>
  ...
  <BCard as="section" aria-label="BFileUploader examples" class="mt-4">
    ...
    <BField
      hint="Demo: this uploader intentionally allows only 1 byte, so selecting a normal file will trigger an inline error."
      id="dev-bfield-photo-error"
      label="Upload error demo"
    >
      {#snippet children({ id, describedBy, invalid, required })}
        <BFileUploader
          accept="image/*"
          aria-describedby={describedBy}
          buttonLabel="Trigger upload error"
          {id}
          {invalid}
          maxFileSizeBytes={1}
          name="photo-error"
