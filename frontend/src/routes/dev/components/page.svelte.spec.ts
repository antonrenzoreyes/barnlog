...
const getTodayLabel = (): string => dateLabelFormatter.format(new Date());

describe("/dev/components/+page.svelte rendering", () => {
  it("renders BInput examples", async () => {
    render(Page);
  ...
  it("renders accessible BField error messaging", async () => {
    render(Page);

    const tagError = page.getByText("Tag must be at least 2 characters.");
    await expect.element(tagError).toBeVisible();
    await expect.element(tagError).toHaveAttribute("role", "alert");
  });

  it("renders BDate default value", async () => {
    render(Page);

    const defaultDateLabel = dateLabelFormatter.format(new Date(2026, 2, 10));
    const dateTrigger = page.getByText(defaultDateLabel);
...
describe("/dev/components/+page.svelte required propagation", () => {
  it("forwards required from BField to underlying controls", async () => {
    render(Page);
    ...
    const tagInput = page.getByPlaceholder("e.g., G-12");
    await expect.element(tagInput).not.toHaveAttribute("required", "");
  });
});
...
describe("/dev/components/+page.svelte file uploader", () => {
  it("renders mobile uploader button and status text", async () => {
    ...
    const uploaderErrorDemoButton = uploaderRegion.getByRole("button", {
      name: /Upload error demo/,
    });

    await expect.element(uploaderButton).toBeVisible();
    await expect.element(uploaderErrorDemoButton).toBeVisible();
