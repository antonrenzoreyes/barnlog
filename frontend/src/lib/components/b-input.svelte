...
<input
  ...
  {autocomplete}
  class="
    h-14 w-full rounded-xl border border-(--ui-color-border)
    bg-(--ui-color-surface) px-3.5 text-base font-medium text-(--ui-color-text)
    shadow-(--ui-shadow-input)
    transition-[border-color,box-shadow,background-color] duration-200 ease-out
    outline-none
    placeholder:font-normal placeholder:text-(--ui-color-placeholder)
