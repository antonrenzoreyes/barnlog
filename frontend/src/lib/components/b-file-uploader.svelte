<script lang="ts">
  ...
  const addCandidateFile = (
    ...
    try {
      ...
      props.onUploadError?.(getErrorMessage(error));
      return false;
    }
  };
  ...
  const collectAcceptedFiles = (nextFiles: File[]): File[] => {
    ...
    nextFiles.forEach((file) => {
      addCandidateFile(nextUploader, file);
    });

    return getSelectedFiles(nextUploader);
  };
  const handleInputChange = (event: Event): void => {
    ...
    const acceptedFiles = collectAcceptedFiles(nextFiles);
